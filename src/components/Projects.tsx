          ...
          {projects.filter(p => p.featured).map((project, index) => (
            <div key={project.title} className="soft-card p-8 group hover:scale-105 transition-all duration-300">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-green-200">{project.title}</h3>
                  <span className="text-sm text-green-400">{project.date}</span>
                </div>
              </div>
          ...
          {projects.filter(p => !p.featured).map((project, index) => (
            <div key={project.title} className="soft-card p-6 group hover:scale-105 transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-green-200">{project.title}</h3>
                  <span className="text-xs text-green-400">{project.date}</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {project.tech.slice(0, 3).map((tech) => (
                  <span
                    key={tech}
                    className="px-2 py-1 bg-green-500/20 text-green-300 rounded-lg text-xs font-medium"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <div className="flex gap-3">
                <a
                  href={project.github}
                  className="flex items-center gap-1 text-green-400 hover:text-green-300 transition-colors duration-200"
                >
                  <Github size={16} />
